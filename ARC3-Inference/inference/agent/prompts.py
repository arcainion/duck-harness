...
STRUCTURED_RUNTIME_STATE_ADDENDUM = (
    ...
    "- Each `history[i].frame` is the frame after `history[i].action`; each frame exposes `.ascii`, `.step`, `.level`, `.shape`, `.segmentation`, `.crop(...)`, `.cell(...)`, `.neighbors(...)`, `.find(...)`, `.color_summary(...)`, `.shortest_path(...)`, `.shortest_path_to_any(...)`, `.color_transitions(...)`, and `.diff(...)`.\n"
...
PYTHON_ADDENDUM = (
    ...
    "- Use `current_frame.color_summary(limit=16)` for a compact dominant-first palette: per-color counts/fractions, inclusive bounds, and edge-contact counts, plus unknown/omitted totals. This is the fastest way to identify likely backgrounds, rare markers, and border/HUD colors.\n"
    "- Batch related bounded queries with `current_frame.analyze([{'method': 'color_summary'}, {'method': 'bounds', 'args': ['R']}])`; each query accepts optional `args` and `kwargs`, and at most 32 queries run in order. Prefer one compound inspection snippet over repeated Python tool calls.\n"
    "- Spatial helpers remove common geometry boilerplate: `bounds(symbols)`, `region_summary([top,left,bottom,right])`, `row_profile(symbol=None)`, `column_profile(symbol=None)`, `nearest_cell(start, symbols, metric='manhattan')`, `distance(start, end, metric='manhattan')`, `line_between(start, end)`, `translate_cells(cells, delta)`, `mirror_cells(cells, symmetry)`, and `compare_regions(first_bounds, second_bounds, allow_recolor=True)`. Results are bounded and letter-coded; bounds passed to region methods are half-open.\n"
    "- Layout/topology helpers provide `border_summary(thickness=1)`, `corner_summary(size=1)`, `center_summary(radius=0)`, `quadrant_summary()`, `color_adjacency(diagonal=False)`, `distance_between_colors(first, second, metric='manhattan')`, `divider_lines(symbol=None)`, `panels(symbol=None)`, `tile_summary(tile_height, tile_width)`, and `edge_distance(symbols)`. Use them to detect HUD borders, grids, panels, tile structure, color contacts, and edge-relative targets without manual board scans.\n"
    "- Use `current_frame.runs(symbol=None, directions='HV', min_length=2, limit=64)` to detect maximal same-color lines. Directions use `H`, `V`, and/or `D`; results include inclusive endpoints, deltas, lengths, per-direction counts, and per-color counts even when run details are truncated.\n"
    "- Use `current_frame.rectangles(symbol=None, kind='any', min_size=2, limit=64)` to detect maximal monochrome filled boxes and closed rectangular outlines. Pass a symbol to exclude background-like regions; results report bounds, shape, fill ratio, edge contact, and interior color counts.\n"
    "- Use `current_frame.enclosed_regions(symbol=None, diagonal=False, limit=64, cell_limit=128)` to find arbitrary-shaped same-color regions that do not reach the frame edge. Results include boundary color counts, geometry, centroids, and bounded cell samples; pass a symbol to focus on likely cavities or contained objects.\n"
    ...
    "- Use `current_frame.object_relations(background=None, diagonal=False, object_limit=32, relation_limit=64)` for bounded pairwise comparisons between multi-color objects. It reports relative directions, row/column overlap, blank-cell gaps, bounding-box containment, centroid deltas, repeated signatures, equal sizes, and shared colors.\n"
    "- Use `current_frame.track_objects(previous_frame, background=None, diagonal=False, limit=64)` to match translation-invariant multi-color objects across frames. It reports moved, unchanged, added, and removed objects with displacement vectors; repeated identical objects are paired by nearest centroid.\n"
    "- Use `current_frame.transform_relation(previous_frame, allow_recolor=True)` to score identity, rotations, and reflections from an earlier frame to the current frame. It reports exact matches, consistent recolor matches and color maps, plus mismatch-ranked candidates for approximate relations.\n"
    "- Use `current_frame.symmetry(sample_limit=8)` to detect exact or near horizontal-axis, vertical-axis, diagonal, and rotational self-symmetries. Candidates report mismatch ratios and bounded cell/counterpart samples that identify symmetry-breaking cells.\n"
    "- Use `current_frame.periodicity(candidate_limit=8, sample_limit=8)` to detect repeated rows, columns, stripes, and tiling periods. It reports exact and fundamental periods, best near-periods, complete-tile status, match ratios, and bounded mismatch pairs.\n"
    "- Use `current_frame.find_pattern(pattern, wildcard=None, transforms=False, max_mismatches=0, mismatch_limit=8, limit=64)` to locate a small letter-coded template. Pass rows such as `['AB', 'BA']`; set a non-color wildcard such as `'?'`, use `transforms=True` for unique rotations/reflections, or set `max_mismatches` to rank partial/corrupted matches with bounded mismatch evidence. Matches use half-open bounds and report their transform.\n"
    "- Use `current_frame.reachable_region(start, passable=..., diagonal=False, max_nodes=4096, cell_limit=128, frontier_limit=64)` to summarize all space reachable from `[row, col]`. It reports maximum distance, farthest cells, bounds, edge contact, and the colors/cells blocking the reachable frontier.\n"
    ...
    "- Use `current_frame.ray(row, col, direction, stop_at=None, include_start=False, limit=64)` for bounded line-of-sight scans in any cardinal or diagonal direction. Optional stop colors are included as `hit`; results report distance, edge reach, and truncated samples.\n"
    "- Use `current_frame.shortest_path(start, goal, passable='...')` for bounded BFS over caller-selected letter-coded colors. `goal` may be one coordinate, up to 64 coordinates, or target symbols such as `'R'`/`['R', 'Y']`; target cells are always enterable. The result includes the selected goal, distance, a bounded path, moves, next_step, and truncation flags.\n"
    ...
    "- If generated code fails, inspect the structured `diagnostic` fields (`type`, `line`, `column`, `source`, nearby `context`, `suggestions`, and `hint`), apply the closest concrete repair, and retry. Earlier definitions do not survive into the retry.\n"
    ...
    "- Tools: use `inspect` for compact standard runtime summaries, `action` for direct validated action batches, and `python` for custom bounded analysis or combined inspect/act programs.\n"
    "- Prefer `inspect` or `action` when they fully express the operation; use Python when custom computation is actually needed.\n"
    "- Inspect structured runtime state rather than reasoning from the raw board by eye.\n"
    ...
    "- Use `current_frame.color_transitions(previous_frame, include_unchanged=False, limit=64, cell_limit=128)` or `last_transition.color_transitions(...)` to aggregate action effects into source-to-target color rules. It reports transition counts and bounds, deterministic or branching mappings, dominant targets, and appearance/disappearance through shape changes.\n"
    ...
    "- Identical expensive structural calls on the same frame are memoized within one Python snippet and return independent copies, so repeated evidence checks avoid rescanning the grid.\n"
