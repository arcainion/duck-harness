...
STRUCTURED_RUNTIME_STATE_ADDENDUM = (
    ...
    "- The raw numeric grid is intentionally not exposed. Use `current_frame.segmentation` as your primary view of the board -- objects, colors, shapes, containment, adjacency, and cross-frame object hashes. Use `current_frame.crop(top, left, bottom, right)` for a small letter-coded region with half-open bounds; do not scan the whole board.\n"
    ...
    "- Each `history[i].frame` is the frame after `history[i].action`; each frame exposes `.ascii`, `.step`, `.level`, `.shape`, `.segmentation`, `.crop(...)`, `.cell(...)`, `.neighbors(...)`, `.find(...)`, `.shortest_path(...)`, `.shortest_path_to_any(...)`, and `.diff(...)`.\n"
...
PYTHON_ADDENDUM = (
    ...
    "- Use `current_frame.find(symbol, limit=64)` to locate a letter-coded color without parsing the full board. It returns the total count, a bounding box, and at most 256 sampled `[row, col]` coordinates.\n"
    "- Use `current_frame.components(symbol, diagonal=False, limit=64, cell_limit=128)` for bounded connected components of one letter-coded color. Each component reports size, bounding box, centroid, edge contact, and sampled cells.\n"
    "- Use `current_frame.objects(background=None, diagonal=False, limit=64, cell_limit=128)` to group adjacent non-background colors into multi-color objects. The modal color is inferred when background is omitted; pass one or more symbols explicitly when needed. Results include palettes and compact letter-coded patterns where `.` marks cells outside the object.\n"
    "- Use `current_frame.find_pattern(pattern, wildcard=None, transforms=False, limit=64)` to locate a small letter-coded template. Pass rows such as `['AB', 'BA']`; set a non-color wildcard such as `'?'`, and use `transforms=True` to include unique rotations and reflections. Matches use half-open bounds and report their transform.\n"
    "- Use `current_frame.cell(row, col)` for one color symbol and `current_frame.neighbors(row, col, diagonal=False)` for four or eight adjacent letter-coded cells with directions and deltas.\n"
    "- Use `current_frame.shortest_path(start, goal, passable='...')` for bounded BFS over caller-selected letter-coded colors. The goal cell is always enterable; the result includes distance, a bounded path, moves, next_step, and truncation flags.\n"
    "- Use `current_frame.shortest_path_to_any(start, goals, passable='...')` to select the nearest of up to 64 candidate coordinates in one BFS. The result includes all requested goals and `selected_goal`.\n"
    "- When `segmentation` is not enough, use `current_frame.crop(top, left, bottom, right)` to read a small letter-coded region. Bounds are half-open, clipped to the frame, and limited to 256 cells.\n"
    "- Every `python` tool call starts fresh. Re-import modules or re-define custom utility logic. Bounded JSON data saved with `remember(key, value)` remains available in `memory` across calls within this game run; use `forget(key)` or `forget()` to remove one key or clear it.\n"
    "- If generated code fails, inspect the structured `diagnostic` fields (`type`, `line`, `column`, `source`, and `hint`), correct the code, and retry. Earlier definitions do not survive into the retry.\n"
    ...
    "- For the most recent change, call `current_frame.diff(previous_frame)` or `last_transition.diff()` for a bounded summary containing change counts, a bounding box, and sampled color-symbol transitions. `history[-1].frame` is the current frame, so comparing it to `current_frame` only compares the board to itself.\n"
    ...
    "- A final Python expression is returned automatically, notebook-style. Use `print(...)` for compact summaries or assign `result` explicitly when needed.\n"
    "- Call `action(...)` inside Python rather than returning action text in the chat.\n"
    "- `action(...)` accepts an ordered list of up to 12 actions. Once your code has selected a reliable sequence, it is often useful to batch it.\n"
